...
use node::{Node, NodeType, string_to_node_type};
use connection::NodeConnection;
use serde_json::json;

use crate::node::node_type_to_string;

#[pyclass]
struct ShortestRouteResolver {
...
type GetNodeResult = (String, usize, String, usize);
...
impl ShortestRouteResolverWrapper {
    ...
    fn get_node(&self, node_id: usize) -> PyResult<GetNodeResult> {
        ...
        Ok((node.name.clone(), node.node_id, node_type_to_string(&node.node_type), node.node_usage))
    }

    fn add_node(&mut self, name: &str, node_type: &str, node_usage: usize) -> PyResult<usize> {
        let node_type = string_to_node_type(&node_type);
        Ok(self.resolver.add_node(name.to_string(), node_type, node_usage))
    }
    ...
    fn set_node_usage(&mut self, node_id: usize, node_usage: usize) -> PyResult<()> {
        self.resolver.set_node_usage(node_id, node_usage);
        Ok(())
    }
