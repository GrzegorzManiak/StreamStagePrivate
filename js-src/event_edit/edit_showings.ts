...
export function handle_showings_panel(panel: HTMLElement) {
    ...
    var add_showing_btn = document.querySelector('#add-showing-btn') as HTMLElement;

    if (add_showing_btn)
        manage_add_showing_btn(add_showing_btn);
...
function append_showing(showing: Showing) {
    ...
    showings_panel.querySelector(`.remove-showing-btn[data-sid="${showing.showing_id}"]`)
...
async function add_showing(
    ...
    create_toast(
        'success', 'Event',
        'Added new event showing successfully.'
    )
}
...
function showing_html(
    ...
    if (showing.venue.length > 0){
        var formatted_location = `${showing.venue} - ${showing.city}`;
    } else {
        var formatted_location = `${showing.city}`;
    }
